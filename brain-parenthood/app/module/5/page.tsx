...
export default function Module5Page() {
  const [currentStep, setCurrentStep] = useState<StepType>(() => { try { return isModuleCompleted(5) ? 'review' : 'overview'; } catch { return 'overview'; } });
    ...
    <AppLayout>
      ...
      <div style={{ backgroundColor: '#F5F7FA', minHeight: 'calc(100vh - 300px)' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 80px' }}>
          ...
          {currentStep === 'assessment' && <AssessmentStep onNext={() => handleSetStep('goals')} onBack={() => handleSetStep('overview')} moduleId={5} />}
          {currentStep === 'goals' && <GoalsStep onNext={() => handleSetStep('review')} onBack={() => handleSetStep('assessment')} moduleId={5} />}
          {currentStep === 'review' && <ReviewStep moduleId={5} onConfirm={() => handleSetStep('complete')} onBack={() => handleSetStep(isCompleted ? 'overview' : 'goals')} isReadOnly={isCompleted} />}
