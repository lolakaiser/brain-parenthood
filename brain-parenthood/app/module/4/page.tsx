...
export default function Module4Page() {
  const [currentStep, setCurrentStep] = useState<StepType>(() => { try { return isModuleCompleted(4) ? 'review' : 'overview'; } catch { return 'overview'; } });
    ...
    <AppLayout>
      ...
      <div style={{ backgroundColor: '#F5F7FA', minHeight: 'calc(100vh - 300px)' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 80px' }}>
          ...
          {currentStep === 'review' && <ReviewStep moduleId={4} onConfirm={() => handleSetStep('complete')} onBack={() => handleSetStep(isCompleted ? 'overview' : 'goals')} isReadOnly={isCompleted} />}
...
function GoalsStep({ onNext, onBack, moduleId }: { onNext: () => void; onBack: () => void; moduleId: number }) {
  ...
  const handleNext = () => {
    ...
    if (currentQuestion < questions.length - 1) { setCurrentQuestion(currentQuestion + 1); } else { saveModuleAnswers(moduleId, 'goals', { ...goals, _labeled }); completeModule(moduleId); onNext(); }
  };
  const handlePrevious = () => {
    if (currentQuestion > 0) { setCurrentQuestion(currentQuestion - 1); } else { onBack(); }
  };
