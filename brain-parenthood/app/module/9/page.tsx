...
export default function Module9Page() {
  const [currentStep, setCurrentStep] = useState<StepType>(() => { try { return isModuleCompleted(9) ? 'review' : 'overview'; } catch { return 'overview'; } });
    ...
    <AppLayout>
      ...
      <div style={{ backgroundColor: '#F5F7FA', minHeight: 'calc(100vh - 300px)' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 80px' }}>
          ...
          {currentStep === 'review' && <ReviewStep moduleId={9} onConfirm={() => handleSetStep('complete')} onBack={() => handleSetStep(isCompleted ? 'overview' : 'goals')} isReadOnly={isCompleted} />}
