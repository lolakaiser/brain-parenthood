    ...
    <AppLayout>
      ...
      <div style={{ backgroundColor: '#F5F7FA', minHeight: 'calc(100vh - 300px)' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 80px' }}>
          ...
          {currentStep === 'assessment' && <AssessmentStep onNext={() => { setAssessmentStartQ(0); handleSetStep('goals'); }} onBack={() => { setAssessmentStartQ(0); handleSetStep('overview'); }} moduleId={12} initialQuestion={assessmentStartQ} />}
          {currentStep === 'goals' && <GoalsStep onNext={() => { setGoalsStartQ(0); handleSetStep('review'); }} onBack={() => { setGoalsStartQ(0); handleSetStep('assessment'); }} moduleId={12} initialQuestion={goalsStartQ} />}
          {currentStep === 'review' && <ReviewStep moduleId={12} onConfirm={() => { completeModule(12); handleSetStep('complete'); }} onBack={() => handleSetStep(isCompleted ? 'overview' : 'goals')} isReadOnly={isCompleted} onEdit={handleEdit} />}
      ...
      {moduleAnswers && (
        <div style={{ marginBottom: '40px' }}>
          <AIInsightCard
            ...
            userData={{ moduleId, moduleName: 'Module 12: Anger Management', answers: moduleAnswers, userName: user?.name, baseline, userGoals, priorModuleGoals }}
